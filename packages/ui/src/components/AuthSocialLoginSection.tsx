...
export interface AuthSocialLoginSectionProps {
  className?: string;
}
...
const PROVIDERS = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" aria-hidden className="w-4.5 h-4.5">
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" aria-hidden className="w-4.5 h-4.5 fill-current">
...
export function AuthSocialLoginSection({ className }: AuthSocialLoginSectionProps) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <p className="text-sm text-muted-foreground/65 mt-1.5 mb-6">
      ...
      <div className="grid sm:grid-cols-2 gap-2.5">
        {PROVIDERS.map((p) => (
          <button
            ...
            aria-label={p.label}
            className={cn(
              "flex items-center justify-center gap-2.5 py-2.5 px-4 text-xs font-semibold rounded-xl border shadow-sm transition-all hover:brightness-95 hover:shadow-md",
              p.bg,
              p.text,
              p.border,
            )}
      ...
      <div className="my-6 flex items-center gap-3">
        ...
        <span className="text-xs uppercase tracking-wider text-muted-foreground/45">or</span>
        <div className="h-px flex-1 bg-foreground/[0.07]" />
      </div>

      <a
        href="/"
        className="block w-full py-2.5 text-xs font-semibold rounded-xl bg-secondary border border-foreground/[0.06] hover:brightness-110 transition-all"
      >
        Sign in with email
      </a>

      <p className="text-xs text-muted-foreground/55 mt-5 leading-relaxed">
        By continuing you agree to our{" "}
        <a href="/" className="font-semibold text-foreground/80 hover:underline">Terms</a> and{" "}
        <a href="/" className="font-semibold text-foreground/80 hover:underline">Privacy Policy</a>.
      </p>
    </div>
  );
}
