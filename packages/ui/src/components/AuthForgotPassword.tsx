...
import { Mail, ArrowRight, ArrowLeft, KeyRound, MailCheck } from "lucide-react";
...
export function AuthForgotPassword({ className }: AuthForgotPasswordProps) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "mx-auto h-12 w-12 rounded-2xl flex items-center justify-center mb-4 border",
          sent ? "bg-emerald-500/10 border-emerald-500/25 text-emerald-400" : "bg-primary/10 border-primary/25 text-primary",
        )}
      >
        {sent ? <MailCheck className="w-6 h-6" aria-hidden /> : <KeyRound className="w-6 h-6" aria-hidden />}
      </div>
      ...
      {sent ? (
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="mt-5 text-left space-y-3">
            <div>
              <label className="block text-xs font-semibold text-muted-foreground/80 mb-1.5" htmlFor="fp-email">
                Email address
              </label>
          ...
          <a href="/" className="inline-flex items-center gap-1 text-xs font-semibold text-muted-foreground/60 hover:text-foreground mt-4 transition-colors">
