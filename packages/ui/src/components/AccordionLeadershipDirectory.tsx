...
import { ChevronDown, Award, Mail, FolderOpen } from 'lucide-react';
import { cn } from '../lib/cn';
import { usePrefersReducedMotion } from '../animation/hooks';

export interface AccordionLeadershipDirectoryProps {
  className?: string;
}

export const AccordionLeadershipDirectory: React.FC<AccordionLeadershipDirectoryProps> = ({ className }) => {
  const [expandedIndex, setExpandedIndex] = useState<number | null>(0);
  const prefersReducedMotion = usePrefersReducedMotion();

  const tiers = [
    { name: "Dr. Evelyn Vane", role: "CHIEF AI ARCHITECT", dept: "R&D", exp: "12 years", email: "evelyn@core.ai", bg: "from-purple-950/20 to-indigo-950/20 border-purple-500/20" },
    { name: "Team Member", role: "PRINCIPAL SECURITY", dept: "CYBER", exp: "9 years", email: "marcus@core.ai", bg: "from-cyan-950/20 to-blue-950/20 border-cyan-500/20" }
  ];

  return (
    <div className={cn('relative w-full max-w-[320px] h-[340px] bg-secondary/5 rounded-3xl border border-border flex flex-col justify-between p-5 overflow-hidden select-none', className)}>
      <span className="text-[9px] text-muted-foreground font-black tracking-widest uppercase">Leadership Accordion</span>

      <div className="flex flex-col gap-3 w-full my-auto">
        {tiers.map((leader, idx) => {
      ...
      <span className="text-[8px] text-muted-foreground font-extrabold uppercase tracking-wide text-center">Click rows to expand directories elastically</span>
    </div>
  );
};
