...
const DEFAULT_QUOTES: MarqueeQuote[] = [
  { id: "q1", quote: "We replaced three internal libraries with Lerpa UI in a week. The token system is the cleanest I have ever shipped against.", name: "Mara Choi", role: "Staff Engineer", company: "Northwind", avatarColor: "oklch(0.72 0.18 280)", rating: 5 },
  { id: "q2", quote: "Reduced-motion was hooked up everywhere by default. Our a11y review caught zero motion bugs on the new pages.", name: "Jules Aramide", role: "Design Systems Lead", company: "Helios", avatarColor: "oklch(0.75 0.15 200)", rating: 5 },
  { id: "q3", quote: "Static export with Next 16 was the unlock. We ship the docs to a CDN now and the LCP dropped under one second.", name: "Sage Okonjo", role: "Frontend Architect", company: "Kestrel", avatarColor: "oklch(0.78 0.16 340)", rating: 5 },
  { id: "q4", quote: "The registry workflow is brilliant — we copy what we need, transform our own tokens on top, and own the source forever.", name: "Iris Vela", role: "Product Engineer", company: "Beacon", avatarColor: "oklch(0.7 0.18 140)", rating: 5 },
  { id: "q5", quote: "Motion choreography across blocks just works. I no longer have to negotiate between designers and reduced-motion users.", name: "Linus Wei", role: "Senior Frontend", company: "Tessera", avatarColor: "oklch(0.74 0.17 60)", rating: 4 },
  { id: "q6", quote: "Type-safe props everywhere, sensible defaults, and zero unwanted dependencies. This is what design systems should feel like.", name: "Ami Nakajima", role: "Engineering Manager", company: "Drift", avatarColor: "oklch(0.76 0.14 320)", rating: 5 },
];
...
function MarqueeRow({ quotes, direction, speed, reduced, pauseOnHover }: RowProps) {
  ...
  return (
    ...
    >
      <motion.div
        animate={reduced ? { x: 0 } : { x: direction === "left" ? ["0%", "-50%"] : ["-50%", "0%"] }}
        transition={{ duration: speed, ease: "linear", repeat: reduced ? 0 : Infinity }}
          ...
          >
            ...
            <figcaption className="flex items-center gap-3 border-t border-border/60 pt-4">
              <span
                ...
                style={{ background: q.avatarColor ?? "oklch(0.7 0.18 280)" }}
              >
                {initials(q.name)}
              </span>
              ...
              {typeof q.rating === "number" ? (
                <div className="flex items-center gap-0.5 text-amber-500" aria-label={`${q.rating} out of 5 stars`}>
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} className={cn("h-3 w-3 fill-current", i < q.rating! ? "text-amber-500" : "text-muted/30")} aria-hidden />
                  ))}
                </div>
              ) : null}
            </figcaption>
    ...
    <section className={cn("relative w-full overflow-hidden bg-background py-20 sm:py-28", className)}>
      ...
      <div className="relative">
        ...
        <div className="flex flex-col gap-4">
          <MarqueeRow quotes={top} direction="left" speed={speed} reduced={reduced} pauseOnHover={pauseOnHover} />
          <MarqueeRow quotes={bottom} direction="right" speed={speed * 1.15} reduced={reduced} pauseOnHover={pauseOnHover} />
        </div>
      </div>
    </section>
  );
