...
const DEFAULT_FEATURES: SpotlightFeature[] = [
  {
    ...
    description: "Every block ships with a strong Props interface and sensible defaults. Drop one into your route, override the props, and you're done.",
    bullets: ["30+ production-ready blocks", "Sensible defaults render out of the box", "Override anything via typed props"],
  ...
  },
  {
    ...
    description: "Framer Motion sequences are gated by prefers-reduced-motion automatically. You don't have to think about it.",
    bullets: ["Reduced-motion gates baked in", "Spring-tuned easings out of the box", "Lazy-loaded animation primitives"],
  ...
  },
  {
    ...
    description: "Theme everything from one CSS-first @theme block. The registry transforms your imports automatically when components are copied out.",
    bullets: ["Single CSS-first token block", "OKLCH color, fluid type, generous radii", "Registry-compatible import rewrites"],
  ...
  },
  {
    ...
    description: "Start with copy-paste source. When you outgrow it, wire up the CLI and pull a curated subset. Same code either way.",
    bullets: ["Copy source straight into your repo", "CLI install for curated subsets", "Zero runtime dependency on us"],
...
export function FeatureSpotlightSwitcher({
  ...
  const accent = active.accent ?? "oklch(0.7 0.18 280)";
    ...
    <section className={cn("relative w-full bg-background py-20 sm:py-28", className)}>
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive ? "border-transparent bg-card shadow-md" : "border-border bg-card/40 hover:border-border/70 hover:bg-card"
                  )}
                  style={isActive ? { boxShadow: `0 0 0 1px ${accent}33, 0 8px 24px ${accent}22` } : undefined}
                >
                  <span
                    className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg"
                    style={{ background: isActive ? accent : "var(--muted, oklch(0.96 0 0))", color: isActive ? "white" : "var(--muted-foreground)" }}
                    aria-hidden
                  >
                    {Icon ? <Icon className="h-4 w-4" /> : <span className="h-1.5 w-1.5 rounded-full bg-current" />}
                  </span>
                  <span className="min-w-0">
                    <span className={cn("block text-sm font-bold", isActive ? "text-foreground" : "text-foreground/80")}>{f.label}</span>
          ...
          <div id={`spotlight-panel-${active.id}`} role="tabpanel" aria-labelledby={`spotlight-tab-${active.id}`} className="lg:col-span-8">
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex flex-col justify-center gap-4">
                  {active.tag ? (
                    <span className="inline-flex w-fit items-center rounded-full bg-primary/10 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary">{active.tag}</span>
                  ) : null}
                  <h3 className="text-balance text-2xl font-bold leading-tight tracking-tight text-foreground sm:text-3xl">{active.title}</h3>
                  <p className="text-pretty text-sm leading-relaxed text-muted-foreground">{active.description}</p>
