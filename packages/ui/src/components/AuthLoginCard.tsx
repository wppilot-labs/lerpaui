...
export function AuthLoginCard({ className }: AuthLoginCardProps) {
  ...
  return (
    ...
    >
      <div className="text-center mb-5">
        ...
        <p className="text-sm text-muted-foreground/65 mt-0.5">Sign in to your account to continue</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        {PROVIDERS.map((p) => (
          <button
            key={p.id}
            type="button"
            className="flex items-center justify-center gap-2 py-2 text-xs font-semibold rounded-xl bg-secondary border border-foreground/[0.06] hover:brightness-110 transition-all"
          >
            {p.icon}
            {p.label}
          </button>
      ...
      <form onSubmit={(e) => e.preventDefault()} className="space-y-3">
        <div>
          <label className="block text-xs font-semibold text-muted-foreground/80 mb-1.5" htmlFor="login-email">
            Email
          </label>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-xs font-semibold text-muted-foreground/80" htmlFor="login-password">
              Password
            </label>
        ...
        <button
          type="submit"
          className="w-full py-2.5 text-xs font-bold rounded-xl bg-primary text-primary-foreground hover:brightness-110 transition-all flex items-center justify-center gap-1.5"
        >
          Sign in
