...
export function NavbarSaasBasic({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden
        className="pointer-events-none absolute inset-x-12 -top-px h-px bg-gradient-to-r from-transparent via-primary/60 to-transparent"
      />
      ...
      <div className="flex items-center justify-between gap-4">
        <a href="/" className="group flex items-center gap-2.5">
          ...
          <span className="text-sm font-semibold tracking-tight">
            Lerpa<span className="text-primary">.</span>
          </span>
          <span className="hidden sm:inline-flex items-center rounded border border-white/10 bg-white/[0.03] px-1.5 py-0.5 text-[10px] font-mono text-muted-foreground/70">
            v0.3
          </span>
        </a>
            ...
            <ul className="pt-3 flex flex-col gap-0.5">
              {LINKS.map((l) => (
                <li key={l.label}>
                  <button
                    role="menuitem"
                    className="w-full flex items-center justify-between rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-white/[0.04] hover:text-foreground"
                  >
                    {l.label}
              ...
              <li className="mt-2 grid grid-cols-2 gap-2">
                <button className="h-9 rounded-lg border border-white/10 text-sm text-foreground">
                  Sign in
                </button>
                <button className="h-9 rounded-lg bg-primary text-sm font-semibold text-bg">
                  Start free
                </button>
              </li>
            </ul>
