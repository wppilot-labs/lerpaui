...
export interface CtaWaitlistGlowProps {
  ...
  title?: string;
  description?: string;
  /** Initial visible counter (will tick up gently while in view). */
  initialCount?: number;
  /** How many people the counter approaches, used to throttle ticking. */
  targetCount?: number;
  placeholder?: string;
  ctaLabel?: string;
  successLabel?: string;
  /** Avatar / initials shown beside the counter line. */
...
export function CtaWaitlistGlow({
  ...
  // Gentle ticker — only when motion is allowed
  React.useEffect(() => {
    if (reduced) return;
    ...
    <section className={cn("relative isolate w-full overflow-hidden bg-background py-20 sm:py-28", className)}>
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        <motion.div
          ...
          style={{
            background:
              "radial-gradient(closest-side, var(--accent, oklch(0.75 0.16 260)) 0%, transparent 70%)",
            filter: "blur(70px)",
          }}
        />
      </div>
      ...
      <div className="mx-auto flex max-w-2xl flex-col items-center px-6 text-center">
        ...
        {error ? (
          <p id={`${id}-err`} role="alert" className="mt-2 text-xs font-medium text-destructive">
            {error}
          </p>
        ) : null}
          ...
          <div className="flex -space-x-2" aria-hidden>
            {avatars.slice(0, 4).map((a) => (
              <span
                ...
                style={{ background: a.color ?? "var(--accent, oklch(0.7 0.18 280))" }}
              >
                {initialsOf(a.name)}
              </span>
            ))}
