...
export interface MagneticBentoShowcaseProps
...
const DEFAULT_TILES: MagneticBentoTile[] = [
  ...
  },
  {
    ...
    body: 'Framer Motion 12 with spring-tuned magnet & scroll reveals.',
    icon: Sparkles,
...
function MagneticTile({
  ...
  const rawX = useTransform<number, number>(
    [sectionMouseX, centerX, tileWidth],
    ([mx, cx, w]) => {
  ...
  const rawY = useTransform<number, number>(
    [sectionMouseY, centerY, tileHeight],
    ([my, cy, h]) => {
      if (!magnetEnabled || reduced) return 0;
...
export const MagneticBentoShowcase = React.forwardRef<
  HTMLElement,
  MagneticBentoShowcaseProps
>(function MagneticBentoShowcase(
  ...
  return (
    <section
      ...
      className={cn(
        'relative w-full bg-background py-20 sm:py-24 lg:py-32',
        className,
      )}
      {...rest}
    >
