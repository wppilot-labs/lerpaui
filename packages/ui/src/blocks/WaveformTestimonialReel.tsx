...
export interface WaveformTestimonialReelProps
...
const DEFAULT_TESTIMONIALS: WaveformTestimonial[] = (
  [
    ["wt-1", "Amara Okafor", "Staff Product Engineer", "Northwind Audio", 5,
    ...
    ["wt-2", "Jonas Hellberg", "Design Systems Lead", "Helix Health", 5,
    ...
    ["wt-3", "Priya Raman", "Founder & CTO", "Frequency Labs", 5,
    ...
    ["wt-4", "Mateo Ricci", "Principal Engineer", "Cadence Studio", 4,
      "Copy-paste registry means zero supply-chain risk. We own the source, evolve the tokens, and the upgrade path stays painless."],
    ["wt-5", "Saoirse Kelly", "VP of Design", "Echo Park Media", 5,
      "Every block reads like a senior engineer wrote it on their best day. Tailwind v4 tokens, Framer Motion 12, React 19 — modern, never reckless."],
  ] as const
).map(([id, name, role, company, rating, quote]) => ({
  id, name, role, company, quote, rating: rating as 1 | 2 | 3 | 4 | 5,
}));
...
const Waveform = React.memo(function Waveform({ playing, intersecting, reducedMotion }: WaveformProps) {
      ...
      {heights.map((h, i) => {
        ...
        return (
          <motion.rect
            ...
            transition={reducedMotion ? { duration: 0 } : { type: "spring", stiffness: 260, damping: 22, mass: 0.4 }}
          />
        );
      })}
    </svg>
    ...
    <span role="img" aria-label={`${rating} out of 5 stars`} className="inline-flex items-center gap-0.5 text-amber-500">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          aria-hidden="true"
          className={cn("h-3.5 w-3.5", i < rating ? "fill-amber-500 text-amber-500" : "fill-transparent text-muted-foreground/40")}
        />
      ))}
    </span>
  );
...
function AvatarBubble({ t }: { t: WaveformTestimonial }) {
  ...
  return (
    <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full bg-gradient-to-br from-primary/20 to-primary/5 ring-2 ring-background">
      {t.avatarUrl ? (
        <img src={t.avatarUrl} alt="" className="h-full w-full object-cover" loading="lazy" decoding="async" />
      ) : (
        <span className="flex h-full w-full items-center justify-center text-sm font-bold text-primary">{initials}</span>
      )}
    </div>
  );
}
...
function Dots({ items, activeIndex, className }: { items: WaveformTestimonial[]; activeIndex: number; className?: string }) {
  return (
    <div className={cn("flex items-center gap-2", className)} aria-hidden="true">
      {items.map((t, i) => (
        <span key={t.id} className={cn(DOT_CLS, i === activeIndex ? "w-6 bg-primary" : "w-1.5 bg-border")} />
      ))}
    </div>
  );
}
...
const TestimonialCard = React.memo(function TestimonialCard({
  testimonial, isActive, isPlaying, reducedMotion, onTogglePlay, onActivate,
}: CardProps) {
  ...
  return (
    <motion.article
      ...
      initial={false}
      animate={reducedMotion ? { opacity: isActive ? 1 : 0.85 } : { opacity: isActive ? 1 : 0.7, scale: isActive ? 1 : 0.97 }}
    ...
    >
      ...
      <header className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 id={authorId} className="truncate text-sm font-semibold leading-tight text-foreground">{testimonial.name}</h3>
          ...
          <div className="mt-1.5"><RatingStars rating={testimonial.rating} /></div>
        </div>
      </header>
      ...
      <blockquote className="mt-5 flex-1 text-[0.95rem] leading-relaxed text-foreground/90">
        ...
        <cite className="sr-only">
          {testimonial.name}, {testimonial.company ? `${testimonial.role} at ${testimonial.company}` : testimonial.role}
        </cite>
      </blockquote>
      ...
      <div className="mt-6 flex items-center gap-3 rounded-xl border border-border/70 bg-background/60 p-3">
        <button
          ...
          onClick={() => { onActivate(testimonial.id); onTogglePlay(testimonial.id); }}
...
export function WaveformTestimonialReel({
  ...
  React.useEffect(() => { if (reducedMotion) setIsPlaying(false); }, [reducedMotion]);

  const activeIndex = React.useMemo(
    () => Math.max(0, safeTestimonials.findIndex((t) => t.id === activeId)),
  ...
  React.useEffect(() => {
    ...
    const id = window.setTimeout(() => {
      ...
      if (nextId) { setActiveId(nextId); scrollToIndex(next); }
    }, autoAdvanceMs);
    return () => window.clearTimeout(id);
  }, [activeIndex, autoAdvanceMs, isPlaying, reducedMotion, safeTestimonials, scrollToIndex, userInteracted]);
    ...
    >
      <div className="mx-auto max-w-6xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 id={headingId} className="text-balance text-3xl font-bold tracking-tight text-foreground sm:text-4xl">{title}</h2>
          {description ? (
          ...
          */}
          ...
          <div className="mt-6 hidden items-center justify-between md:flex">
            ...
            <div className="flex items-center gap-2">
              <button type="button" aria-label="Previous testimonial" onClick={() => step(-1)} className={NAV_BTN_CLS}>
                <ChevronLeft className="h-4 w-4" aria-hidden="true" />
              </button>
              <button type="button" aria-label="Next testimonial" onClick={() => step(1)} className={NAV_BTN_CLS}>
